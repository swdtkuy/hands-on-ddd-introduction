...
describe("Rating", () => {
    ...
    test("should throw an error for non-integer values", () => {
        ...
        expect(() => new Rating("3" as any)).toThrow("Rating value must be an integer.");
    });

    test("should throw an error for values below the minimum", () => {
        expect(() => new Rating(0)).toThrow("Rating value must be between 1 and 5.");
    });

    test("should throw an error for values above the maximum", () => {
        expect(() => new Rating(6)).toThrow("Rating value must be between 1 and 5.");
    });
});