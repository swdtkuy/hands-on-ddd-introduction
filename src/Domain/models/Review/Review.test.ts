...
describe("Review", () => {
  ...
  describe("equals", () => {
    test("should be equal when reviewIds are the same", () => {
      const r1 = Review.create(makeIdentity("id-1"), makeBookId(), makeName(), makeRating(3));
      const r2 = Review.create(makeIdentity("id-1"), makeBookId(), makeName(), makeRating(5));
      expect(r1.equals(r2)).toBe(true);
    });

    test("should not be equal when reviewIds differ", () => {
      const r1 = Review.create(makeIdentity("id-1"), makeBookId(), makeName(), makeRating(3));
      const r2 = Review.create(makeIdentity("id-2"), makeBookId(), makeName(), makeRating(3));
  ...
  describe("isTrustworthy", () => {
    describe("without comment", () => {
      test("should return true when rating quality factor meets threshold", () => {
        ...
        const review = Review.create(makeIdentity(), makeBookId(), makeName(), makeRating(4));
    ...
    describe("with comment", () => {
      test("should return true when combined factor meets threshold", () => {
        ...
        const review = Review.create(
          makeIdentity(), makeBookId(), makeName(), makeRating(5), makeComment(longComment),
        );
        expect(review.isTrustworthy()).toBe(true);
      });
      ...
      test("should return false when combined factor is below threshold", () => {
        ...
        const review = Review.create(
          makeIdentity(), makeBookId(), makeName(), makeRating(1), makeComment("短い"),
        );
        expect(review.isTrustworthy()).toBe(false);
      });
    });
  ...
  describe("extractRecommendedBooks", () => {
    ...
    test("should extract book titles from comment", () => {
      const review = Review.create(
        makeIdentity(), makeBookId(), makeName(), makeRating(4),
        makeComment("『ドメイン駆動設計』を読む。「クリーンアーキテクチャ」もおすすめです。"),
      );
      expect(review.extractRecommendedBooks()).toEqual(
    ...
    test("should deduplicate repeated book titles", () => {
      const review = Review.create(
        makeIdentity(), makeBookId(), makeName(), makeRating(5),
  ...
  describe("mutations", () => {
    test("updateName should update the name", () => {
      const review = Review.create(makeIdentity(), makeBookId(), makeName("旧名前"), makeRating(3));
